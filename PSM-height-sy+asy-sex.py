...
# 若高度数据有独立输入文件，替换此处路径；若无则保持原路径
input_file = r"E:\PG After24-9\论文\冉宇-C7倾斜角\数据处理\spine公开代码及数据备用\PSM length&height\PSM height filter combine - 6 columns.xlsx"
output_dir = r"E:\PG After24-9\论文\冉宇-C7倾斜角\数据处理\spine公开代码及数据备用\PSM length&height"
# 输出文件命名替换为height（若需区分C7S，可改为PSM-C7S-sy+asy-sex.xlsx）
output_file = os.path.join(output_dir, "PSM-height-sy+asy-sex.xlsx")
...
# 列名映射保持不变（第六列是C7S），仅后续取值改为C7S
data.columns = ["Sex", "Age", "Disease_Type", "Cervical_Curvature_Type", "C7_Height", "C7S"]
...
try:
    ...
    # ========== 可选修改：提示文字从C7 Height改为C7S，更贴合实际 ==========
    print(f"✅ Statistical results of C7S have been saved to: {output_file}")
