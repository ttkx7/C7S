...
# Read data
file_path = r"E:\PG After24-9\论文\冉宇-C7倾斜角\数据处理\spine公开代码及数据备用\PSM length&height\PSM length.xlsx"  # Please replace with your file path
data = pd.read_excel(file_path)

# Check if column names meet expectations
expected_columns = ['No', 'Sex', 'Age', 'Symptom status', 'cervical curvature type', 'C7 length', 'C7S', 'propensity_score_age', 'weight_age', 'propensity_score_c7', 'weight_c7']
...
for disease_type in [1, 2]:  # 1 represents Cervical spondylosis group, 2 represents Healthy population group
    ...
    # First filter male and female samples based on propensity_score_c7 and C7 length error of ±0.1
    for idx_male, row_male in case_data[case_data['Sex'] == 0].iterrows():  # Iterate over male samples
        print(f"Processing male sample {row_male['No']} - propensity_score_c7: {row_male['propensity_score_c7']}, C7 length: {row_male['C7 length']}")  # Debug output
        # Filter eligible female samples based on propensity_score_c7 and C7 length error of ±0.1
        female_group_for_matching = case_data[(case_data['Sex'] == 1) & 
                                               (np.abs(case_data['propensity_score_c7'] - row_male['propensity_score_c7']) <= 0.1) & 
                                               (np.abs(case_data['C7 length'] - row_male['C7 length']) <= 0.1)]
        ...
        for idx_female, row_female in female_group_for_matching.iterrows():
            ...
            if age_distance == np.min(age_distance):
                ...
                matched_results.append({
                    'No_Male_c7': row_male['No'],  # Male sample
                    ...
                    'C7_length_Male_c7': row_male['C7 length'],  # C7 length for male
                    ...
                    'No_Female': selected_match['No'],  # Female sample
                    ...
                    'C7_length_Female_c7': selected_match['C7 length'],  # C7 length for female
...
output_path = r"E:\PG After24-9\论文\冉宇-C7倾斜角\数据处理\spine公开代码及数据备用\PSM length&height\PSM length filter.xlsx"  # Please replace with the path to save the file
